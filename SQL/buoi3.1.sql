...
select title,film.description,category.name from category inner join film_category on category.category_id = film_category.category_id 
inner join film on film.film_id = film_category.film_id where film.rating = 'R';
...
inner join actor on actor.actor_id = film_actor.actor_id where first_name = 'NICK' and last_name = 'WAHLBERG';
-- 6.Tìm email của các khách hàng ở Mỹ
select email from customer inner join address on customer.address_id = address.address_id 
inner join city on city.city_id = address.city_id inner join country on country.country_id = city.country_id where country = 'United States';